...
```{r}
imcenfant <- read.csv2("C:/Users/clara/Desktop/imcenfant.csv")
names(imcenfant)
```
...
```{r}
zep.enfant <- imcenfant$SEXE[imcenfant$IMC < 15 & imcenfant$age <= 3.5]
Table(zep.enfant) 
```
...
6.2 Utilisez la fonction \texttt{qqplot()} pour étudier si l'IMC a le même fonction de distribution que une variable aléatoire normal. 
...
n <-length(z)
N <- rnorm(n)
qqplot(z,N,xlim=c(-3,3),ylim=c(-3,3),xlab ="taille std",ylab = "Normal" )
par(new=T)
plot(z,z,type="l",col="red",xlim=c(-3,3),ylim=c(-3,3),xlab = "",ylab = "")
```
...
n <-length(z)
N <- rnorm(n)
qqplot(z,N,xlim=c(-3,3),ylim=c(-3,3),xlab ="poids std",ylab = "Normal" )
par(new=T)
plot(z,z,type="l",col="red",xlim=c(-3,3),ylim=c(-3,3),xlab = "",ylab = "")
```
